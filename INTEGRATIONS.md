...
**cookieaudit** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `cookieaudit` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
cookieaudit ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
cookieaudit ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
cookieaudit ... --format json | cognis-connect emit --to sigma   # Sigma rules
cookieaudit ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
```
...
works whatever `cookieaudit` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="cookieaudit") for rec in records]   # records = your JSON output
