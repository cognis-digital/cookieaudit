"""Native cognis-connect emit for cookieaudit — forward findings to any platform.

Maps cookieaudit's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    cookieaudit ... --format json | cookieaudit-emit --to stix
    cookieaudit-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('cookie', '') + ' ' + rec.get('flag', '')
        out['severity'] = 'low' if 'secure' in rec.get('flag', '') else 'info'
        out['type'] = 'security'
        out['description'] = f"Cookie '{rec.get('cookie', '')}' has flag '{rec.get('flag', '')}'"
        out['tags'] = ['cookie', 'security']
