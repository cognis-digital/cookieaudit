...
[![PyPI](https://img.shields.io/pypi/v/cognis-cookieaudit.svg)](https://pypi.org/project/cognis-cookieaudit/)
[![CI](https://github.com/cognis-digital/cookieaudit/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/cookieaudit/actions)
...
COOKIEAUDIT exists for one job — audit set-cookie flags (secure/httponly/samesite) from a response dump — and does it without a SaaS bill or heavyweight setup.
...
```bash
cookieaudit --version
cookieaudit scan .                      # scan the current project
cookieaudit scan . --format json
cookieaudit scan . --fail-on high       # non-zero exit for CI gates
cookieaudit mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
`cookieaudit` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
