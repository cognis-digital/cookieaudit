...
- **rules/analyzers** apply the heuristics shipped in `cookieaudit/core.py`.
- **score** ranks by severity.
- **MCP server** (`cookieaudit mcp`) exposes `scan` for Cognis.Studio agents.
